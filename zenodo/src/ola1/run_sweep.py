...
def main():
    ...
    engine_config = args.engine_config or (args.config_dir / "engine_core3.json")
    layer_states = args.layer_states or (args.config_dir / "layer_states.yaml")
    engine_cfg = _load_engine_config(engine_config)
    engine_cfg_path = str(engine_config) if engine_config else None
    engine_cfg_hash = None
    if engine_config and engine_config.exists():
        import hashlib

        engine_cfg_hash = hashlib.sha256(engine_config.read_bytes()).hexdigest()
    ...
    if engine_cfg:
        ...
        peak_selector_frac_struct = 0.08
        peak_selector_snr_min = 5.0
    layer_cfg = _load_layer_state_config(layer_states)
    family_spec = None
    family_fp = None
    family_priors = None
