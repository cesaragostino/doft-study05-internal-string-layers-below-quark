...
def main() -> None:
    ...
    parser.add_argument("--selection-config", type=Path, default=None)
    ...
    promoted_dir = processed_dir / "promoted"
    promoted_blocks = promoted_dir / "simple_blocks.json"
    promoted_dna = promoted_dir / "dof_dna_catalog.csv"
    ...
    # Mirror promoted outputs into a normalized folder for downstream use.
    promoted_dir.mkdir(parents=True, exist_ok=True)
    if simple_blocks_json.exists():
        shutil.copy2(simple_blocks_json, promoted_blocks)
    if dna_catalog.exists():
        shutil.copy2(dna_catalog, promoted_dna)
